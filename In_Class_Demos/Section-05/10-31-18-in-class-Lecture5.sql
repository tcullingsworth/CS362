...
SELECT Province, ISNULL(min(Population),0) as MinPopulation, ISNULL(max(Population),0) as MaxPopulation
FROM dbo.City  			
GROUP BY Province;
WHERE MinPopulation > 0;
...
SELECT row_number() over(order by e.GDP*1000000.0/c.Population DESC) as row_num,
c.Name, 
			  c.Population, 
			  e.GDP*1000 AS GDP, 
			  e.GDP*1000000.0/c.Population AS [GDP-per-Capita]
...
WHERE Country = 'USA'
GROUP BY Province
HAVING SUM(Population)>10000 AND
Province = 'Colorado';

-- summing the population and reporting on population greater than 100000
